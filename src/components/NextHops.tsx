...
// Styled by pages/nexthop.css (multi-route sheet, id "venue game"); GameRows by
// the shared gamerow.css.
...
export function GamesThatWeekend({ anchorDate, city, league, excludeGameKey, excludeVenueName }: {
  ...
  return (
    <section className="block"><div className="container">
      ...
      <div className="nh-sub">{fmtDay(win.days[0])} – {fmtDay(win.days[2])}{city ? ` · ${city} first` : ''} · <Link to="/weekend" className="nh-link">full weekend slate →</Link></div>
      {list === null ? <div className="nh-empty">Loading games…</div> : null}
      {list && !list.length ? <div className="nh-empty">Nothing else on the slate that weekend.</div> : null}
...
export function RelatedExperience({ g }: { g: Game }) {
  ...
  return (
    <section className="block"><div className="container">
      ...
      <h2 className="shead">How this trip rates</h2>
      <Link to="/rankings" search={{ q: hit.m.name }} className="nh-exp">
        <span className="nh-exp-lab"><img src="/img/logo.png" alt="" width={18} height={18} /> Snapback Score</span>
        <span className="nh-exp-val">{hit.m.final.toFixed(1)}</span>
        <span className="nh-exp-sub">#{hit.m.rank} in America · {hit.m.name} · see the rankings →</span>
      </Link>
    </div></section>
  )
}
...
export function NearbyVenues({ city, state, excludeId, excludeName }: {
  ...
  return (
    <section className="block"><div className="container">
      ...
      <div className="nh-sub">{city ? `Around ${city}` : state ? `Around ${state}` : ''}</div>
      {venues === null ? <div className="nh-empty">Loading venues…</div> : null}
      <div className="nh-cards">
        {list.map((v) => {
          const img = cardImg(v.image)
          const logo = v.teams[0]?.logo
          return (
            <Link key={v.id} to="/venue" search={{ id: v.id }} className="nh-vc" {...intentWarm(() => { if (img) warmImage(img) })}>
              <span className={'nh-vc-im' + (img ? '' : ' logo')}>
                {img ? <img src={img} alt="" loading="lazy" /> : logo ? <img className="lg" src={logo} alt="" loading="lazy" /> : null}
              </span>
              <span className="nh-vc-m">
                <span className="t">{v.name}</span>
                <span className="d">{[v.city, v.state].filter(Boolean).join(', ')}</span>
              </span>
            </Link>
          )
        })}
