...
const uk = (m: ScoreInput) => `${m.dateISO}|${m.home}|${m.away}`
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      for (const m of need) {
        ...
        try {
          const r = await fetch(`/api/match-score?date=${m.dateISO}&home=${encodeURIComponent(m.home!)}&away=${encodeURIComponent(m.away!)}`)
